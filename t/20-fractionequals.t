...
tests sameNumeratorAndDenominator => sub {
    is_deeply( fraction(3, 5), fraction(3, 5), 'adding same two fractions');
};
...
tests differentDenominators => sub {
    cmp_ok( fraction(3, 4), 'ne', fraction(3, 7), 'different denominators');
};

tests wholeNumberEqualsSameFraction => sub {
    is_deeply( fraction(5, 1), fraction(5), 'whole number equals same fraction');
};
